...
export default function ContactPage() {
  ...
  return <>
    ...
    <section className="bg-gradient-to-br from-green-950 via-green-800 to-emerald-700 px-4 py-16 text-white md:py-24">
      <div className="mx-auto max-w-5xl text-center"><p className="font-semibold uppercase tracking-[0.18em] text-green-100 text-sm">Mumbai, Thane & online across India</p><h1 className="mt-4 text-4xl font-bold tracking-tight md:text-6xl">Contact DietFiniti</h1><p className="mx-auto mt-6 max-w-3xl text-lg leading-8 text-green-50 md:text-xl">Ask about a personalised nutrition consultation at our Mumbai or Thane clinics, or an online consultation from anywhere in India.</p></div>
    </section>
    ...
    <main className="bg-slate-50 py-16 md:py-20"><div className="mx-auto grid max-w-6xl gap-10 px-4 lg:grid-cols-[.9fr_1.1fr]">
      <aside className="space-y-6">
        ...
        <a href="tel:+919321057899" className="flex gap-4 rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-200 transition hover:ring-green-400"><Phone className="mt-1 text-green-700" aria-hidden="true"/><span><strong className="block text-slate-900">Call DietFiniti</strong><span className="mt-1 block text-slate-700">+91 93210 57899</span></span></a>
        <a href="https://wa.me/919321057899" target="_blank" rel="noopener noreferrer" className="flex gap-4 rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-200 transition hover:ring-green-400"><MessageCircle className="mt-1 text-green-700" aria-hidden="true"/><span><strong className="block text-slate-900">WhatsApp</strong><span className="mt-1 block text-slate-700">Send a consultation enquiry directly.</span></span></a>
        <a href="mailto:dietfiniti@gmail.com" className="flex gap-4 rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-200 transition hover:ring-green-400"><Mail className="mt-1 text-green-700" aria-hidden="true"/><span><strong className="block text-slate-900">Email</strong><span className="mt-1 block text-slate-700">dietfiniti@gmail.com</span></span></a>
        <div className="flex gap-4 rounded-xl bg-green-50 p-5 ring-1 ring-green-100"><Clock className="mt-1 text-green-700" aria-hidden="true"/><span><strong className="block text-slate-900">Clinic hours</strong><span className="mt-1 block text-slate-700">Monday–Friday: 11:00 AM–8:00 PM<br/>Saturday: 11:00 AM–6:00 PM</span></span></div>
      </aside>

      <section className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200 md:p-8" aria-labelledby="enquiry-heading"><h2 id="enquiry-heading" className="text-2xl font-bold text-slate-900">Send a consultation enquiry</h2><p className="mt-2 text-slate-700">This form opens WhatsApp with your enquiry; you can review it before sending.</p><form onSubmit={submit} className="mt-7 grid gap-5"><label className="grid gap-2 font-medium text-slate-800">Name<input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="rounded-lg border border-slate-300 px-3 py-3 font-normal outline-none ring-green-600 focus:ring-2" /></label><label className="grid gap-2 font-medium text-slate-800">Phone number<input required type="tel" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} className="rounded-lg border border-slate-300 px-3 py-3 font-normal outline-none ring-green-600 focus:ring-2" /></label><div className="grid gap-5 sm:grid-cols-2"><label className="grid gap-2 font-medium text-slate-800">What do you need help with?<select value={form.service} onChange={(e) => setForm({ ...form, service: e.target.value })} className="rounded-lg border border-slate-300 bg-white px-3 py-3 font-normal outline-none ring-green-600 focus:ring-2">{["Weight-loss nutrition support", "Medical weight-management support", "Pregnancy nutrition", "Weight gain", "Sports nutrition", "Bridal nutrition", "Other"].map((x) => <option key={x}>{x}</option>)}</select></label><label className="grid gap-2 font-medium text-slate-800">Preferred option<select value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} className="rounded-lg border border-slate-300 bg-white px-3 py-3 font-normal outline-none ring-green-600 focus:ring-2">{["Online consultation", "Lower Parel clinic, Mumbai", "Thane clinic", "Not sure yet"].map((x) => <option key={x}>{x}</option>)}</select></label></div><label className="grid gap-2 font-medium text-slate-800">Message <span className="font-normal text-slate-500">(optional)</span><textarea value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} rows={4} className="resize-y rounded-lg border border-slate-300 px-3 py-3 font-normal outline-none ring-green-600 focus:ring-2" /></label><button type="submit" className="rounded-lg bg-green-700 px-5 py-3 font-semibold text-white transition hover:bg-green-800">Continue on WhatsApp</button>{notice && <p role="status" className="text-sm text-green-800">{notice}</p>}</form></section>
    </div></main>

    <section className="bg-white px-4 py-16 md:py-20"><div className="mx-auto max-w-6xl"><h2 className="text-3xl font-bold text-slate-900">Visit DietFiniti in Mumbai or Thane</h2><p className="mt-3 max-w-3xl leading-7 text-slate-700">Please contact the team before visiting to ask about appointment availability and the most suitable consultation option.</p><div className="mt-8 grid gap-6 md:grid-cols-2">{locations.map((location) => <article key={location.name} className="rounded-xl border border-slate-200 p-6"><MapPin className="text-green-700" aria-hidden="true"/><h3 className="mt-4 text-xl font-bold text-slate-900">{location.name}</h3><address className="mt-3 not-italic leading-7 text-slate-700">{location.address}</address><a href={location.map} target="_blank" rel="noopener noreferrer" className="mt-5 inline-block font-semibold text-green-800 underline underline-offset-4">Open in Google Maps</a></article>)}</div></div></section>
        ...
        <ol className="mt-10 grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            <li key={n} className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
              <span className="inline-flex h-9 w-9 items-center justify-center rounded-full bg-green-700 font-bold text-white">{n}</span>
        ...
        <div className="mt-7 divide-y divide-slate-200 rounded-xl border border-slate-200 bg-white">
          {[
            ...
            <details key={q} className="group p-5">
              <summary className="cursor-pointer list-none pr-8 font-semibold text-slate-900">{q}<span className="float-right text-green-700 group-open:rotate-45">+</span></summary>
