    ...
    <section aria-labelledby="dietitian-mumbai-heading" className="bg-white py-16 md:py-20">
      <div className="mx-auto max-w-6xl px-4">
        <div className="grid gap-10 lg:grid-cols-[1.1fr_.9fr] lg:items-start">
          <div>
            <p className="mb-3 font-semibold uppercase tracking-wider text-green-700 text-sm">Mumbai, Thane & online across India</p>
            ...
            <div className="mt-4 rounded-xl bg-green-50 border border-green-200 p-5 text-base leading-7 text-slate-800">
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              <Link href="/online-dietitian-consultation" className="rounded-lg bg-green-700 px-5 py-3 font-semibold text-white transition hover:bg-green-800">Explore online consultations</Link>
              <Link href="/contact" className="rounded-lg border border-green-700 px-5 py-3 font-semibold text-green-800 transition hover:bg-green-50">Contact DietFiniti</Link>
            </div>
          </div>
          <aside className="rounded-2xl border border-green-100 bg-green-50 p-7" aria-label="Who DietFiniti can help">
            <h3 className="text-xl font-bold text-slate-900">How DietFiniti can help</h3>
            <ul className="mt-5 space-y-4">
              {links.map((item) => <li key={item.href} className="leading-6 text-slate-700"><Link href={item.href} className="font-semibold text-green-800 underline decoration-green-300 underline-offset-4 hover:text-green-950">{item.title}</Link> {item.text}</li>)}
            </ul>
          </aside>
        </div>
      </div>
