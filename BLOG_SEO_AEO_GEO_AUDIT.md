...
7. **Images** — ⚠️ **Owner decision (2026-08-05): the blog uses no images** (covers removed, `coverImage` dropped from `posts.json`, `public/image/blog/` deleted). Do not add images back without owner approval. If images are ever reintroduced: descriptive alt text, WebP/AVIF compression, `next/image` with proper sizes, covers at `public/image/blog/{slug}.jpg`.
